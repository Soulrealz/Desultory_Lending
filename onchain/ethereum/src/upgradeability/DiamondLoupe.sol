...
contract DiamondLoupeFacet {
        ...
        for (uint256 selectorIndex; selectorIndex < selectorCount; selectorIndex++) {
            ...
            for (uint256 facetIndex; facetIndex < numFacets; facetIndex++) {
                if (facets[facetIndex].facetAddress == facetAddress) {
                    facets[facetIndex].functionSelectors[numFacetSelectors[facetIndex]] = selector;                                   
    ...
    function facetFunctionSelectors(address _facet) external view returns (bytes4[] memory functionSelectors) {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        uint256 selectorCount = ds.selectors.length;
